...
// service later widens would still leak (a `primarySignal` refined from
// "asn_spam" to "asn_spam:AS55967/Baidu/76ip" is exactly the detail stripped
// below). Every forwarded string is therefore length-capped.
const botStr = function(v) {
...
const botFieldsFromResponse = function(body) {
  ...
  if (botStr(o.band)) out.band = botStr(o.band);
  if (botStr(o.primarySignal)) out.primarySignal = botStr(o.primarySignal);
  ...
  // would abort the whole /aGTM.js response. Capped — the list is
  // service-controlled and ends up inline in every response.
  if (o.signals && typeof o.signals === 'object' && typeof o.signals.length === 'number') {
    const sig = [];
    for (let i = 0; i < o.signals.length && sig.length < 10; i++) {
      const s = o.signals[i];
      if (s && typeof s === 'object') {
        const e = {};
        if (botStr(s.type)) e.type = botStr(s.type);
        if (botStr(s.category)) e.category = botStr(s.category);
        if (typeof s.score === 'number') e.score = s.score;
...
if (botCheckEnabled && botCheckUrl) {
  if (!clientIP) {
    ...
    sendHttpGet(botCheckUrl + '/' + b64Payload, {timeout: 5000}).then(function(r) {
      ...
      if (CFG.debug) logToConsole('debug', 'Bot check verdict', {status: r.statusCode, mode: CFG.botCheckMode, bot: botState.verdict});
      ...
      botState.verdict = {isBot: false, band: 'unknown'};
      afterBotCheck(false);
    });
  }
} else {
...
const buildAndSend = function(sessionData) {
  ...
  // botCheckExpose is opt-out (absent === on), so an existing config keeps
  // publishing; a tenant who wants filtering WITHOUT telling the page can
  // uncheck it. The verdict is a classification of this visitor written into a
  // global object before any consent decision, so that choice has to exist.

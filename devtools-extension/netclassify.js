...
(function (root) {
  ...
  var TRACKERS = [
    ...
    // ID Sync for Microsoft's Conversions API. Microsoft's docs require it to run
    // CLIENT-side (a server call cannot see the browser context it needs) and
    // recommend firing it at least once per session; it is mandatory only for
    // audience/remarketing use. Either way it is a third-party pixel on a site
    // that may otherwise look purely server-side.

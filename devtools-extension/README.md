...
- **Integration management (demo/prospect flows):** a **block** box neutralises a
  ...
  isolation; an **inject** box runs a pasted aGTM integration snippet at global scope
  via a `<script>` element — it needs no existing aGTM, so the Simulation tab renders
  even on a page where `window.aGTM` is absent.

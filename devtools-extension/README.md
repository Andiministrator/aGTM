...
- **Cookie reset + reload** — expires cookies whose name matches one of the given
  ...
  see at a glance whether your CMP was covered (the shipped list leads with prefixes such as `__cmp`, which covers Consentmanager's whole `__cmpconsent…`/`__cmpccu…` family; a run that matches nothing says so instead of reporting success, and — with **"auch in Drittanbieter-Frames (CMP)"** on — the same reset also runs *inside* the third-party CMP frames. That matters: a CMP like Consentmanager keeps its own copy of the consent state in its own origin (cookies on `.consentmanager.net` plus a `localStorage` under `cdn.consentmanager.net`), which the page cannot touch, so a top-frame-only reset lets the CMP restore everything and the banner never reappears. DevTools can reach a frame via `inspectedWindow.eval({frameURL})` **without any manifest permission**; the frames are discovered from `getResources()`. The effect line reports what each pass removed and survives the reload it triggers) (across the `/` + current-path × parent-domain grid), optionally clears

...
**Error signal (opt-in):** with `cfg.fallbackEvent` enabled, the library fires `aGTM_repeat_fallback` into the dataLayer **only** on the timeout-fallback path — i.e. when the wait-event(s) never arrived and the replay ran unenriched. Nothing is emitted on a normal (enriched) replay. It carries `aGTMrepeatCount` (may be `0` — e.g. a guest with nothing to replay; the event still signals the fallback ran) and `aGTMrepeatSource`; trigger a monitoring/alert tag on it to catch missing enrichment. The event starts with `aGTM` so it bypasses consent and is never itself replayed. Off by default; needs a fallback timeout > 0.

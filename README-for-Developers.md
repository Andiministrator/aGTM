...
| `_noConsent` | boolean | `true` bypasses the consent gate entirely. The event is pushed to the dataLayer and POST is sent without waiting for consent. The property is preserved in the dataLayer event. See [`_noConsent` flag](#_noconsent-flag). **Not from an iFrame** unless the "aGTM iFrame Support" tag in the top frame opts in — the tag drops every key with a leading `_` out of a `postMessage` (`_post`/`_post_sent`/`_noDLPush` unconditionally). |

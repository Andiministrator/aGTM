...
<!-- ───────────────────────────────────────────────────────────────
     ARBEITSPROTOKOLL FÜR CLAUDE CODE (aBoard-Framework)
     Interne Doktrin auf Deutsch (ADR-0002). Der englische Produkt-
     & Architektur-Guide beginnt weiter unten bei "## Project Summary".
     ─────────────────────────────────────────────────────────────── -->

## Arbeiten mit Claude Code (aBoard-Framework)

> Dieser Abschnitt ist die **interne Arbeits-Doktrin** (Deutsch). Die
> öffentliche Produktsprache von aGTM ist **Englisch** (Code, Commits,
> README, Produkt-Doku) — siehe `docs/adr/0002-projektsprache.md`. Alles
> ab „## Project Summary" bleibt davon unberührt.

### Vor jeder Antwort — Pflicht

**AKTION VOR ANTWORT.** Vor jeder Antwort prüfen (gilt auch bei kurzen Acks):

1. **Board-Status** (`aboard-sync.json`, sobald aBoard aktiv): Gibt es eine Karte
   in „In Arbeit" / „Bereit", die meine Antwort betrifft? Auf welche beziehe ich mich?
2. **Findings** (`docs/findings.md`): Offenes `P0`, das mein Schritt berührt?
3. **Open Decisions** (`docs/open-decisions.md`): Hängt der Schritt an einer offenen Entscheidung?
4. **Memory-Regeln** prüfen (z. B. Sprachregel, sGTM-Sandbox-Restriktionen, CMP-Loader-Muster).
5. **ES5-/Build-Reflex**: Berührt der Schritt Library-/CMP-Code? Dann gilt **ES5-only**
   (`var`, keine Arrow-Functions/Klassen/Template-Literals) und nach Änderung
   `bun test` + `./build.sh` ohne Warnung (Details unten im englischen Guide).

### Sprache (ADR-0002)

- **Öffentlich = Englisch**: `aGTM.js`/`cmp/*`-Code + Kommentare, Commit-Messages,
  `README*.md`, Produkt-/Entwickler-Doku, GitHub-sichtbare Artefakte.
- **Intern = Deutsch**: dieser Abschnitt, `docs/adr/`, `docs/qa/`, `docs/findings.md`,
  `docs/glossar.md`, `docs/open-decisions.md`, `knowledge/`, Board-Kommentare, Memory.
- Code-Bezeichner bleiben englisch (Domänenkonvention), auch in deutschen Texten.

### Board-Felder (aBoard)

aBoard-Karten haben Felder, die du aktiv liest und schreibst:

| Feld | Badge | Bedeutung | Wann setzen |
|---|---|---|---|
| `critical: true` | ⚡ Kritisch | Sprint-kritisch, blockiert Fortschritt | Karte muss dringend erledigt werden, um weiterzukommen |
| `blocked: true` | ⊘ Blockiert | Wartet auf externe Abhängigkeit | Karte kann nicht weitergeführt werden (+ Kommentar mit Grund) |
| `qa: true` | ✓ QA | Karte wurde geprüft | Nach erfolgreichem Test/Review / Sprint-Abschluss |
| `owner` | — | Zuständig: `""` / Username / `"Claude"` / `"Claude Opus"` / … | Delegation via Family-Matching; fremdes Claude-Modell ⇒ Sub-Agent spawnen |

**Konvention:** Karte in „In Arbeit" = Arbeitsauftrag. Nach Erledigung `qa: true` + nach „Erledigt".

#### Felder ändern

**Ziel-Konvention (ADR-0067, ab Board-Migration):** Board-Mutation = Op an
`aboard-sync.inbox/` anhängen — **NIEMALS `aboard-sync.json` direktschreiben**.
`aboard-sync.json` ist projektor-geschrieben und für alle anderen Akteure read-only.
Ops via `makeOp(type, payload, actor)` aus `src/ops.js`, als
`<ISO-ts>-<uuid>.json` in die Inbox. Beispiel:

```json
{ "type": "card.field", "payload": { "cardId": "<id>", "field": "critical", "value": true }, "actor": "<Akteur>" }
```

**Erkennungsmerkmal „migriert":** `aboard-sync.inbox/` **und** `aboard-sync.base.json`
existieren neben `aboard-sync.json`.

> **Caveat (noch nicht migriert):** Solange Inbox + base fehlen, ist das Board nicht
> auf ADR-0067 migriert (Ausgangszustand direkt nach Projekt-Modus-Aktivierung). Dann
> gilt das F-283-Direktschreib-Schutzprotokoll: Browser-Tab quieszieren ODER Konflikt
> per „Disk-Version" lösen (NIE „Meine Version"), atomarer Single-Write — und
> `aboard-sync.json` darf direkt bearbeitet werden. Migration jederzeit per
> `node ~/projects/aBoard/tools/aboard-projektor.mjs --dir ~/projects/aGTM` (idempotent).

#### Kommentare hinzufügen

Migriert: `comment.add`-Op an die Inbox. Nicht migriert: direkt ins `comments`-Array
der Karte (unter F-283-Schutz). Payload:

```json
{ "type": "comment.add", "payload": { "cardId": "<id>", "comment": { "id": "<uuid>", "author": "<Akteur>", "text": "...", "created": "YYYY-MM-DD" } }, "actor": "<Akteur>" }
```

### Session-Einstieg (`/projekt-start`)

1. `aboard-sync.json` lesen (Board-Stand); falls kein Tab offen, headless projektieren:
   `node ~/projects/aBoard/tools/aboard-projektor.mjs --dir ~/projects/aGTM`.
2. `docs/findings.md` (offene `P0`?) + `docs/open-decisions.md` sichten.
3. Letzte Commits (`git log --oneline -8`), Branch beachten (`dev` = Entwicklung).
4. Memory-Aktualität gegen das reale Repo prüfen (Memories können veraltet sein).
5. **Schätz-Check**: Claude-zugewiesene Karten ohne `complexity`/`effort`? ⇒ vor
   Arbeitsbeginn schätzen + `agentContext` kuratieren.

### Sub-Agents

Sub-Agents (QA, Kritiker, Recherche) haben **keinen Memory-Zugriff**. Ihre Prompts
müssen **selbst-enthaltend** sein — alle Regeln + Kontext explizit oder als Dateipfade
(`knowledge/topics/projekt-konventionen.md` als Referenz mitgeben). Nie implizit auf
Memory verweisen. Board-Mutation auf nicht-migriertem Board macht nur der Hauptchat.

### Autonomie & Arbeitsweise (3 K's)

Routine autonom entscheiden und ausführen, nicht jede Reihenfolge abstimmen.
Rückfragen nur bei echten Weichen (Halluzinationsrisiko, schwer reversibel,
Sicherheits-/Datenverlust). **3 K's vor jedem Schritt:** Korrektheit, Konsistenz,
Klarheit. Nicht-triviale Antworten enden mit einem Block „was muss ICH (Andi) tun".

### QA & Qualitätssicherung

- Schritt-QA: `docs/qa/schritt-qa.md` — pro Arbeitsschritt (ES5, `bun test` grün,
  `./build.sh` ohne Warnung, Doku-Sync bei Core-Func-Änderungen).
- Sprint-Abschluss: `docs/qa/sprint-abschluss.md` — Abhängigkeits-Check + Kritiker-Sub-Agent.
- **Kein `v*`-Release-Tag ohne `/sprint-abschluss`** inkl. Kritiker (ADR-0062 sinngemäß).
- Blocker: [S]purensicherung immer, [K]ritiker bei P0/P1.

### Context-Management

Bei ~50 % Kontextauslastung sauberen Schnitt vorbereiten (`/chat-wechsel`),
damit kein Stand verloren geht.

### Wissenssammlung & Recherche

Recherche-Spurensicherung: `knowledge/spurensicherung.md` +
`knowledge/sources-visited.md` — **vor** erneutem externen Abruf den Quellenindex prüfen.
Domänen-Wissen (CMP, Consent, sGTM) thematisch unter `knowledge/topics/`.

### Doktrin-Verweise

- Entscheidungen: `docs/adr/` (ADR-0001 Format, 0002 Sprache, 0004 QA, 0005 Commits, 0006 Versionierung, 0012 Sub-Agents).
- Glossar: `docs/glossar.md` · Offene Entscheidungen: `docs/open-decisions.md`.
- Projektstatus: `aboard-sync.json` (Single Source of Truth, sobald aBoard aktiv).
- **Commit-Konvention & Versionierung**: English Conventional Commits + SemVer/Git-Tags —
  Details im englischen Guide unten („Git Workflow") sowie `docs/adr/0005`/`0006`.

...
- **Session `window.se_data` fallback.** When `aGTM.d.session` is empty the panel
  looks for a site session object (`window.se_data`, as on example.net) and shows it.
...
- **URL build:** `sources_api_url` holds the bare base **without** tenant/query; the Client appends `/{tenant}` (and the attribution query after it). Help text warns against baking tenant/`?attribution=true` into the field (produces `…/tp/sources/acme/?attribution=true/acme`).

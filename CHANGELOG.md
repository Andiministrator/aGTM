...
- **Source capture (sequential):** the POST is now awaited (changed from fire-and-forget) so its response body's `source` field (e.g. `it_webgains`) can be captured into `sessionData.source`. It flows through `cfg.session.source` into the library's `aGTM.d.session.source` — readable in webGTM via a plain GTM "JavaScript Variable" with path `aGTM.d.session.source`, no custom template. On timeout/error/non-2xx the field is left unset and delivery proceeds. Adds one internal round-trip to /aGTM.js latency.
...
- The aGTM library's only involvement is the preset-gate widening + deep-copy that carry `cfg.session.source` through to `aGTM.d.session.source`; the POST itself is purely server-side.
...
### Attribution read-back (sGTM Client wiring) — removed before release

- The sGTM Client's Attribution API GET (`fireAttribution` + template options `attribution_enabled`/`attribution_methods`/`attribution_api_url`) was added during v1.5 development but **removed before release**: it was end-to-end non-functional (api4sources returns `404` for the read endpoint) and not currently needed. Since v1.5 is unreleased, no migration path is required.
- The **library-side** attribution machinery (`aGTM.f.resolveAttribution`, `aGTM.f.parseUrlParams`, the HYBRID merge in `config()`, `aGTM.d.attribution`) is **kept dormant** — it is harmless without a `cfg.session.attribution` payload, and re-enabling the feature is a pure Client-side re-add (no library change). See the library-side section above.

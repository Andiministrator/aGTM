...
- **Opt-in error signal `aGTM_repeat_fallback`** (checkbox "Fire an error event if the wait-event(s) never arrive"): pushed **only** on the timeout-fallback path (wait-event never came → unenriched replay), with `aGTMrepeatCount` + `aGTMrepeatSource`. Trigger a monitoring/alert tag on it. Nothing is emitted on a normal enriched replay.

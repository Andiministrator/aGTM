...
- **Response capture (sequential):** the POST is now awaited (changed from fire-and-forget) so its response payload can be captured. On 2xx, **every non-meta top-level field** is copied into `sessionData` (meta = `ok`/`tenant`/`session_id`/`ts`/`skipped`/`reason`; reserved session keys `uid`/`sid`/`consent`/… blacklisted so the API can't clobber the session). The flat affiliate `source` (last-cookie-win, e.g. `it_webgains`) lands at `aGTM.d.session.source`, readable in webGTM via a plain GTM "JavaScript Variable", no custom template. On timeout/error/non-2xx nothing is captured and delivery proceeds. Adds one internal round-trip to /aGTM.js latency.
